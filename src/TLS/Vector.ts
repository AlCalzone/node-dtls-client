...
export class Vector<T extends number | ISerializable> extends Array<T> implements ISerializable {
	...
	serialize(): Buffer {
		...
		switch (this.spec.itemSpec.type) {
			...
			case "enum":
				...
				serializedItems = (this as number[]).map(v => numberToBuffer(v, bitSize));
				break;

			case "struct":
				serializedItems = (this as ISerializable[]).map(v => v.serialize());
			}
		...
		switch (this.spec.itemSpec.type) {
			...
			case "enum":
				...
				for (let i = 0; i < length; i += bitSize / 8) {
					this.push(bufferToNumber(buf, bitSize, delta) as any as T); // we know this is a number!
					delta += bitSize / 8;
				}
				break;
			case "struct":
				...
				while (i < length) {
					...
					i += item.readBytes;
					this.push(item.result as any as T); // we know this is a struct/ISerializable
				}
		}
		return delta;
	}
