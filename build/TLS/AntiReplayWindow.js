...
var width = 64; // bits / entries, must be multiple of INT_SIZE
var INT_SIZE = 32; // in JS, bitwise operators use 32bit ints
/**
...
var AntiReplayWindow = /** @class */ (function () {
     ...
     */
    AntiReplayWindow.prototype.mayReceive = function (seq_num) {
     ...
     * @param seq_num - The sequence number of the packet to be checked
     */
    AntiReplayWindow.prototype.hasReceived = function (seq_num) {
        // check if the packet was received already
        var lowerBound = this.ceiling - width + 1;
        // find out where the bit is located
        var bitIndex = seq_num - lowerBound;
        var windowIndex = Math.floor(bitIndex / INT_SIZE);
        var windowBit = bitIndex % INT_SIZE;
        var flag = 1 << windowBit;
        // check if it is set;
     ...
     */
    AntiReplayWindow.prototype.markAsReceived = function (seq_num) {
        if (seq_num > this.ceiling) {
            // shift the window
            var amount = seq_num - this.ceiling;
            // first shift whole blocks
            while (amount > INT_SIZE) {
                for (var i = 1; i < this.window.length; i++) {
